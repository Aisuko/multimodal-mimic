...
def train(args, train_dataloader, test_dataloader):
    ...
    print(f'Training dataset shape {X_train_reshaped.shape}')
    ...
    test(logreg, test_dataloader)


def test(logreg, test_dataloader):
    test_data=[]
    test_labels=[]
    ...
    y_pred_prob=logreg.predict_proba(X_test_reshaped)[:, 1]
